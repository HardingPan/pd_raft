...
class ResidualBlock(nn.Module):
    def __init__(self, in_planes, planes, norm_fn='group', stride=1):
        ...
        '''
        关于Norm
        BatchNorm:统计C_i上所有N张图片(H*W)的均值和方差
        LayerNorm:统计N_i上所有C张图片(H*W)的均值和方差
        InstanceNorm:统计(C_i, N_j)上图片(H*W)的均值和方差
        GroupNorm:统计(C_i1,C_i2,······;N_j)上图片(H*W)的均值和方差,介于<IN,LN>
        '''
        if norm_fn == 'group':
        ...
        if stride == 1:
            ...
            # 定义残差块短路:nn.Sequential是一个有序的容器，神经网络模块将按照在传入构造器的顺序依次被添加到计算图中执行，同时以神经网络模块为元素的有序字典也可以作为传入参数
            # 进行下采样
